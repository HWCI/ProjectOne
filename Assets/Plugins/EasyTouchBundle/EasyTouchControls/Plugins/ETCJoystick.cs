...
[System.Serializable]
public class ETCJoystick : ETCBase, IPointerEnterHandler, IDragHandler, IBeginDragHandler, IPointerDownHandler,
    IPointerUpHandler
{
    #region Unity Events
    ...
    [SerializeField] private bool isNoReturnThumb;

    public bool IsNoReturnThumb
    {
        get { return isNoReturnThumb; }
        ...
        if (joystickType == JoystickType.Dynamic && !_visible && _activated)
        {
            ...
            if (isTouchOverJoystickArea(ref localPosition, ref screenPosition))
            {
                ...
                if (overGO == null || (allowJoystickOverTouchPad && overGO.GetComponent<ETCTouchPad>()) ||
                    (overGO != null && overGO.GetComponent<ETCArea>()))
                {
    ...
    public void OnDrag(PointerEventData eventData)
    {
        if (pointId == eventData.pointerId)
        {
            ...
            if (!isNoReturnThumb)
            {
                thumbPosition =
                    (eventData.position -
                     eventData.pressPosition); // / (cachedRootCanvas.rectTransform().localScale.x  ) ;
            }
            else
            {
                thumbPosition =
                    ((eventData.position - noReturnPosition) / cachedRootCanvas.rectTransform().localScale.x) +
                    noReturnOffset;
            }
            ...
            if (isNoOffsetThumb)
            {
                thumbPosition = (eventData.position - (Vector2) cachedRectTransform.position) /
                                cachedRootCanvas.rectTransform().localScale.x;
            }
    ...
    private void UpdateJoystick()
    {
        ...
        if (axisX.axisValue != 0 || axisY.axisValue != 0)
        {
            ...
            {
                // X axis
                if (axisX.actionOn == ETCAxis.ActionOn.Down &&
                    (axisX.axisState == ETCAxis.AxisState.DownLeft || axisX.axisState == ETCAxis.AxisState.DownRight))
                {
                ...
                // Y axis
                if (axisY.actionOn == ETCAxis.ActionOn.Down &&
                    (axisY.axisState == ETCAxis.AxisState.DownUp || axisY.axisState == ETCAxis.AxisState.DownDown))
                {
    ...
    private bool isScreenPointOverArea(Vector2 screenPosition, ref Vector2 localPosition)
    {
        ...
        {
            if (RectTransformUtility.ScreenPointToLocalPointInRectangle(cachedRootCanvas.rectTransform(),
                screenPosition, null, out localPosition))
            {
                switch (joystickArea)
                {
        ...
        {
            if (RectTransformUtility.RectangleContainsScreenPoint(userArea, screenPosition,
                cachedRootCanvas.worldCamera))
            {
                RectTransformUtility.ScreenPointToLocalPointInRectangle(cachedRootCanvas.rectTransform(),
                    screenPosition, cachedRootCanvas.worldCamera, out localPosition);
                returnValue = true;
            }
        }
    ...
    private void DoTurnAndMove()
    {
        ...
        if (axisX.directTransform != null)
        {
            ...
            if (axisX.directCharacterController != null)
            {
                if (axisX.directCharacterController.isGrounded || !tmLockInJump)
                {
                    Vector3 move = axisX.directCharacterController.transform.TransformDirection(Vector3.forward) *
                                   speed;
