...
{
    public static void AxisInspector(ETCAxis axis, string label, ETCBase.ControlType type, bool turnMove = false,
        string[] unityAxes = null)
    {
        ...
        if (axis.enable)
        {
            ...
            if (axis.showGeneralInspector)
            {
                ETCGuiTools.BeginGroup(20);
                {
                    ...
                    {
                        axis.valueMethod =
                            (ETCAxis.AxisValueMethod) EditorGUILayout.EnumPopup("Dead zone method", axis.valueMethod);
            ...
            if (!turnMove)
            {
                ...
                axis.showDirectInspector =
                    EditorGUILayout.Foldout(axis.showDirectInspector, "Direction ation (optional)");
                if (axis.showDirectInspector)
                {
                    ETCGuiTools.BeginGroup(20);
                    {
                        ...
                        //EditorGUILayout.BeginHorizontal();
                        axis.autoLinkTagPlayer = EditorGUILayout.ToggleLeft("Auto link on tag", axis.autoLinkTagPlayer,
                            GUILayout.Width(200));
                        ...
                        if (!axis.autoLinkTagPlayer)
                        {
                            axis.directTransform = (Transform) EditorGUILayout.ObjectField("Direct action to",
                                axis.directTransform, typeof(Transform), true);
                        }

                        axis.directAction =
                            (ETCAxis.DirectAction) EditorGUILayout.EnumPopup("Action", axis.directAction);
                        if (axis.directAction != ETCAxis.DirectAction.Jump)
                        {
                            axis.axisInfluenced =
                                (ETCAxis.AxisInfluenced) EditorGUILayout.EnumPopup("Affected axis",
                                    axis.axisInfluenced);
                        }
                        else
                        {
                        ...
                        if ((axis.directCharacterController || axis.autoLinkTagPlayer) &&
                            (axis.directAction == ETCAxis.DirectAction.Translate ||
                             axis.directAction == ETCAxis.DirectAction.TranslateLocal))
                        {
                ...
                axis.showInertiaInspector =
                    EditorGUILayout.Foldout(axis.showInertiaInspector, "Gravity-Inertia-smoothing...");
                        ...
                        if (axis.directAction == ETCAxis.DirectAction.RotateLocal)
                        {
                            ...
                            if (axis.isAutoStab)
                            {
                                ...
                                ETCGuiTools.BeginGroup(5);
                                {
                                    ...
                                    axis.autoStabThreshold =
                                        EditorGUILayout.FloatField("Threshold ", axis.autoStabThreshold);
                                }
                                ETCGuiTools.EndGroup();
                                //EditorGUI.indentLevel++;
                            }
