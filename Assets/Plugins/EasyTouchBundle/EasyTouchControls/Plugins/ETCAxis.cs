...
public class ETCAxis
{
    ...
    [SerializeField] private Transform _directTransform;

    public Transform directTransform
    {
        get { return _directTransform; }
            ...
            switch (directAction)
            {
                ...
                case ETCAxis.DirectAction.TranslateLocal:
                    ...
                    else
                    {
                        if (directCharacterController.isGrounded || !isLockinJump)
                        {
                            Vector3 direction = directCharacterController.transform.TransformDirection(localAxis) *
                                                axisSpeedValue;
                            directCharacterController.Move(direction);
                            lastMove = directCharacterController.transform.TransformDirection(localAxis) *
                                       (axisSpeedValue / Time.deltaTime);
                        }
                        else
                        {
                            directCharacterController.Move(lastMove * Time.deltaTime);
                ...
                case ETCAxis.DirectAction.Force:
                    ...
                    {
                        Debug.LogWarning("ETCAxis : " + name + " No rigidbody on gameobject : " +
                                         _directTransform.name);
                    }
    ...
    private void DoAutoStabilisation()
    {
        ...
        if (angle > startAngle - autoStabThreshold || angle < startAngle + autoStabThreshold)
        {
            ...
            {
                case AxisInfluenced.X:
                    stabAngle = new Vector3(axis, _directTransform.localRotation.eulerAngles.y,
                        _directTransform.localRotation.eulerAngles.z);
                    break;
                case AxisInfluenced.Y:
                    stabAngle = new Vector3(_directTransform.localRotation.eulerAngles.x, axis,
                        _directTransform.localRotation.eulerAngles.z);
                    break;
                case AxisInfluenced.Z:
                    stabAngle = new Vector3(_directTransform.localRotation.eulerAngles.x,
                        _directTransform.localRotation.eulerAngles.y, axis);
                    break;
            }
