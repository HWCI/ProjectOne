...
public class ETCDPad : ETCBase, IDragHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    private void GetTouchDirection(Vector2 position, Camera cam)
    {
        ...
        // Left
        if ((localPoint.x < -buttonSize.x / 2 && localPoint.y > -buttonSize.y / 2 && localPoint.y < buttonSize.y / 2 &&
             dPadAxisCount == DPadAxis.Two_Axis)
        ...
        // Up
        if ((localPoint.y > buttonSize.y / 2f && localPoint.x > -buttonSize.x / 2f &&
             localPoint.x < buttonSize.x / 2f && dPadAxisCount == DPadAxis.Two_Axis)
