...
public class EasyTouchInspector : Editor
{
    public override void OnInspectorGUI()
    {
                ...
                if (t.enabledNGuiMode)
                {
                    ...
                    HTGuiTools.BeginGroup(5);
                    {
                        ...
                        for (int i = 0; i < t.nGUICameras.Count; i++)
                        {
                            ...
                            {
                                t.nGUICameras[i] =
                                    (Camera) EditorGUILayout.ObjectField("", t.nGUICameras[i], typeof(Camera), true);
            ...
            HTGuiTools.BeginGroup();
            {
                ...
                if (t.autoSelect)
                {
                    ...
                    t.autoUpdatePickedObject =
                        HTGuiTools.Toggle("Auto update picked gameobject", t.autoUpdatePickedObject, true);
                    EditorGUILayout.Space();
                    ...
                    HTGuiTools.BeginGroup(5);
                    {
                        ...
                        for (int i = 0; i < t.touchCameras.Count; i++)
                        {
                            ...
                            if (i >= 0)
                            {
                                t.touchCameras[i].camera = (Camera) EditorGUILayout.ObjectField("",
                                    t.touchCameras[i].camera, typeof(Camera), true, GUILayout.MinWidth(150));
                                t.touchCameras[i].guiCamera = EditorGUILayout.ToggleLeft("Gui",
                                    t.touchCameras[i].guiCamera, GUILayout.Width(50));
                                EditorGUILayout.EndHorizontal();
                            }
                        }
                    }
        ...
        if (t.showGestureInspector)
        {
            ...
            {
                t.gesturePriority =
                    (EasyTouch.GesturePriority) EditorGUILayout.EnumPopup("Priority to", t.gesturePriority);
                ...
                t.doubleTapDetection =
                    (EasyTouch.DoubleTapDetection) EditorGUILayout.EnumPopup("Double tap detection",
                        t.doubleTapDetection);
        ...
        if (t.showTwoFingerInspector)
        {
            HTGuiTools.BeginGroup();
            {
                ...
                if (t.enable2FingersGesture)
                {
                    ...
                    t.twoFingerPickMethod =
                        (EasyTouch.TwoFingerPickMethod) EditorGUILayout.EnumPopup("Pick method", t.twoFingerPickMethod);
        ...
        if (t.showSecondFingerInspector)
        {
            HTGuiTools.BeginGroup();
            {
                ...
                if (t.enableSimulation)
                {
                    ...
                    t.secondFingerTexture =
                        (Texture) EditorGUILayout.ObjectField("Texture", t.secondFingerTexture, typeof(Texture), true);
                    EditorGUILayout.HelpBox(
                        "Change the keys settings for a fash compilation, or if you want to change the keys",
                        MessageType.Info);
