...
public class HTGuiTools
{
    ...
    {
        if (value) GUI.backgroundColor = Color.green;
    ...
    private static void DrawLine(Color color, int padding = 0)
    {
        ...
        GUI.color = color;
        GUI.DrawTexture(new Rect(padding, lastRect.yMax - lastRect.height / 2f, Screen.width, 1f),
    ...
    private static Rect DrawGradient(int padding, int width, int height = 35)
    {
        ...
        GUI.DrawTexture(new Rect(padding, lastRect.yMin + 1, width, lastRect.yMax - lastRect.yMin),
            GetGradientTexture());
