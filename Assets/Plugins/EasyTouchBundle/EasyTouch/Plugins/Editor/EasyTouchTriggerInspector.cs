    ...
    public override void OnInspectorGUI()
    {
        ...
        for (int i = 1; i < eventNames.Length; i++)
        {
            ...
            int result = t.receivers.FindIndex(
                delegate(EasyTouchTrigger.EasyTouchReceiver e) { return e.eventName == ev; }
            );
        ...
        if (folding)
        {
            ...
            while (i < t.receivers.Count)
            {
                if (t.receivers[i].eventName == ev)
                {
                    ...
                    t.receivers[i].triggerType =
                        (EasyTouchTrigger.ETTType) EditorGUILayout.EnumPopup("Testing on", t.receivers[i].triggerType);

                    EditorGUILayout.BeginHorizontal();
                    t.receivers[i].restricted = EditorGUILayout.Toggle("", t.receivers[i].restricted,
                        (GUIStyle) "Radio", GUILayout.Width(15));
                    ...
                    {
                        t.receivers[i].gameObject = (GameObject) EditorGUILayout.ObjectField("Other object",
                            t.receivers[i].gameObject, typeof(GameObject), true);
                    }

                    EditorGUILayout.Space();
                    EditorGUILayout.Space();

                    t.receivers[i].otherReceiver =
                        HTGuiTools.Toggle("Other receiver", t.receivers[i].otherReceiver, true);
                    if (t.receivers[i].otherReceiver)
                    {
                        t.receivers[i].gameObjectReceiver = (GameObject) EditorGUILayout.ObjectField("Receiver",
                            t.receivers[i].gameObjectReceiver, typeof(GameObject), true);
                    }
                    ...
                    string[] mNames = null;
                    if (!t.receivers[i].otherReceiver ||
                        (t.receivers[i].otherReceiver && t.receivers[i].gameObjectReceiver == null))
                    {
                    ...
                    // Parameter
                    t.receivers[i].parameter =
                        (EasyTouchTrigger.ETTParameter) EditorGUILayout.EnumPopup("Parameter to send",
                            t.receivers[i].parameter);
                    ...
                    if (comp.GetType().IsSubclassOf(typeof(MonoBehaviour)))
                    {
                        ...
                        foreach (MethodInfo methodInfo in methodInfos)
                        {
                            if ((methodInfo.DeclaringType.Namespace == null) ||
                                (!methodInfo.DeclaringType.Namespace.Contains("Unity") &&
                                 !methodInfo.DeclaringType.Namespace.Contains("System")))
                            {
